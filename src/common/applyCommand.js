...
import { Vector3, Matrix, Axis } from "@babylonjs/core";
...
export default (entity, command) => {
  ...
  const matrix = Matrix.RotationAxis(Axis.Y, entity.mesh.rotation.y);
  const direction = Vector3.TransformCoordinates(impulse, matrix);
  ...
  entity.mesh.moveWithCollisions(
    new Vector3(
