...
import { getScene, getCamera } from "./state";
// import SetupPlayerHUD from "../gameClient/HUD/HUDPlayerList";
const gameHttpEndpoint = "http://localhost:2657";
const client = new Colyseus.Client(gameHttpEndpoint);
const worldName = "gallery";
const username = "Daniel ";
// multiplayer
export const JoinOrCreateRoom = () => {
  client
    .getAvailableRooms(worldName)
    .then((rooms) => {
      debugger;
      // create new room/ world if it doesn't exist
      if (rooms.length === 0) {
        Axios.post(`${gameHttpEndpoint}/room/new`, {
          name: worldName,
        }).then(() => {
          InitialRoom(worldName, username);
        });
      } else InitialRoom(worldName, username);
    })
    .catch((err) => alert("error while connecting to server!"));
};
